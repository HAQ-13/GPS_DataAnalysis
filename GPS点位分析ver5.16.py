...
# output filename
output1_path = "./output/2019-05-16-类型一：ΔT异常.csv"
output2_path = "./output/2019-05-16-类型二：ΔD异常.csv"
output3_path = "./output/2019-05-16-类型三：Δv异常.csv"
output4_path = "./output/2019-05-16-类型四：GPS缺失（ΔD和ΔT同时异常）.csv"
output5_path = "./output/2019-05-16-异常总结表.csv"
output6_path = "./output/2019-05-16-类型五：GPS系统跳点（待分析）.csv"

# input filename
filename = "./export/2019-05-16-沪ER5107"
...
for i in range(0, len(df)):
    timeStamp = df.iloc[i, 7] / 1000
    timeArray = time.localtime(timeStamp)
    datetime_list.append(time.strftime("%Y-%m-%d %H:%M:%S", timeArray))
...
print()
result1_df = df[(np.abs(df['delta_T']) != 10000)]
result1_df['type'] = '1'
result1_df.to_csv(output1_path, mode='a', header=False, encoding='gbk')
# 特殊情况：ΔT为0时ΔD不为0
...
print()
result2_df = df[(np.abs(df['delta_D']) > 0.25)]
result2_df['type'] = '2'
result2_df.to_csv(output2_path, mode='a', header=False, encoding='gbk')

# 筛选出Δv的outlier
print("Δv的异常值： ")
print(df[(np.abs(df['delta_v']) > 30)])
print()
result3_df = df[(np.abs(df['delta_v']) > 30)]
result3_df['type'] = '3'
result3_df.to_csv(output3_path, mode='a', header=False, encoding='gbk')

# 筛选出GPS缺失的点（ΔT和ΔD同时较大）
print("GPS数据缺失点（ΔT和ΔD同时较大）： ")
print(df[(np.abs(df['delta_D']) > 0.25) & (np.abs(df['delta_T']) != 10000)])
print()
result4_df = df[(np.abs(df['delta_D']) > 0.25) & (np.abs(df['delta_T']) != 10000)]
result4_df['type'] = '4'
result4_df.to_csv(output4_path, mode='a', header=False, encoding='gbk')

# 筛选出可能的GPS系统跳点（特殊情况）
print("GPS可能系统跳点（待分析）： ")
print(df[(np.abs(df['delta_T']) >= 10000) & (np.abs(df['delta_T']) <= 12000) & (np.abs(df['delta_D']) > 0.25)])
print()
result5_df = df[(np.abs(df['delta_T']) >= 10000) & (np.abs(df['delta_T']) <= 12000) & (np.abs(df['delta_D']) > 0.25)]
result5_df['type'] = '5'
result5_df.to_csv(output6_path, mode='a', header=False, encoding='gbk')

# 异常点的summary表（前四种情况和待分析的第五种情况）
sum1_df = result1_df[['vehicle_license_number', 'datetime', 'type']]
sum2_df = result2_df[['vehicle_license_number', 'datetime', 'type']]
sum3_df = result3_df[['vehicle_license_number', 'datetime', 'type']]
sum4_df = result4_df[['vehicle_license_number', 'datetime', 'type']]
summary_df = pd.concat([sum1_df, sum2_df, sum3_df, sum4_df])
summary_df.to_csv(output5_path, mode='a', header=False, encoding='gbk')
